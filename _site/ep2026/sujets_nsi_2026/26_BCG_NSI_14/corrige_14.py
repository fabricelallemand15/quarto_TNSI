...
class Piece:
    ...
    def ajouter_sortie(self, direction: str, position: int):
        ...
        # lignes ajoutées à la question 3 pour permettre l'ajout de sorties à l'est ou au sud de la pièce
        elif direction == "S":
    ...
    def choix_sortie(self, i: int, j: int) -> tuple:
        ...
        '''
        # QUESTION 4
        # Explications : Deux erreurs dans la méthode d'origine :
        # 1. Erreur logique : `if k < 0` est toujours faux car `k` vaut 1, 2, 3... La condition devrait comparer les distances : `if d2 < distance`.
        # 2. Variable non définie : `d2` n'est jamais calculé. Il faut ajouter `d2 = distance_de_manhattan(autre_sortie)` avant la comparaison.
        ...
        for k in range(1, len(self.sorties)):
            ...
            d2 = distance_de_manhattan(autre_sortie) #ligne ajoutée
            if d2 < distance: # ligne modifiée
                choix = autre_sortie
                distance = d2
        return choix
...
def evacuation(p: Piece, silencieux: bool = True) -> int:
    ''' simule l'évacuation de la pièce et renvoie le nombre de tours nécessaire.
        A chaque tour, chacun des occupants se déplace, si possible, d'une case vers la sortie la plus proche. Si le paramètre silencieux vaut false, l'état de la pièce à chaque tour est affiché dans la console.
        A FAIRE EN PARTIE 1
    '''
    # Question 2
    nb_tours = 0
    while p.nb_occupants_restants() > 0:
        mouvement = p.alerter(silencieux)
        nb_tours += 1
        if not silencieux:
            print(p)
        if not mouvement:
...
def test_nb_occupants_restants():
    ...
    #tests persos ajoutés
    p1 = Piece(5, 7)
    print(p1.nb_occupants_restants())  # 0
    p1.ajouter_occupants(0, 3, 4)
    p1.ajouter_occupants(0, 1, 2)
    p1.ajouter_occupants(3, 4, 3)
    print(p1.nb_occupants_restants())  # 9
...
def test_evacuation(silencieux: bool = True):
    ...
    p1 = Piece(5, 7)
    p1.ajouter_sortie("N", 5)
    p1.ajouter_occupants(0, 3, 3)
    p1.ajouter_occupants(1, 1, 1)
    p1.ajouter_occupants(3, 2, 5)
    nb_tours = evacuation(p1)
    print(f"Évacuation en {nb_tours} tours")
