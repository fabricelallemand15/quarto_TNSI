...
4. Pour évaluer la **complexité** de cet algorithme, nous allons compter le nombre de multiplications et de comparaisons effectuées. Démontrer, à l'aide d'un raisonnement par récurrence, que la complexité de cet algorithme est en $\mathcal{O}(n)$.
...
Un autre constat qui montre l'inefficacité de ce programme : plusieurs calculs identiques sont répétés plusieurs fois. On calcule par exemple $F_3$ deux fois et $F_2$ trois fois. Une meilleure solution serait de garder en mémoire les éléments déjà calculés et de ne calculer que les nouveaux éléments encore jamais rencontrés. Une telle démarche relève de la **programmation dynamique** qui sera abordée en fin d'année.
...
Voir [cet article du blog](https://sitelf.fr/blog/posts/timeexec/){ target=_blank } qui explique comment visualiser le temps d'exécution d'une fonction.
:::
...
::: {.callout-caution}
## Question bonus
Une autre méthode de calcul de $x^n$ consiste à distinguer le cas où $n$ est pair et celui où $n$ est impair :
