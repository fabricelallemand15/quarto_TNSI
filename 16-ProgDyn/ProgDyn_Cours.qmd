...
:::{.callout-important}
## Retrouver le chemin optimal (backtracking)

Les algorithmes précédents calculent la **valeur optimale** `c(sommet)`, mais on peut aussi reconstruire le **chemin** qui réalise cette valeur.

Principe : partir du sommet, puis descendre niveau par niveau en choisissant à chaque étape l'enfant qui a été retenu par le `max`.

- En approche ascendante : une fois le tableau `C` rempli, à la position `(i, j)`, comparer `C[i+1][j]` et `C[i+1][j+1]`.
- Si `C[i+1][j] >= C[i+1][j+1]`, on va à gauche ; sinon, on va à droite.
- Répéter jusqu'au dernier niveau : la suite des positions visitées donne un chemin optimal.

En approche descendante avec mémoïsation, l'idée est la même : après avoir rempli `memo`, on repart de `(0, 0)` et on choisit à chaque niveau l'enfant dont la valeur mémorisée est maximale.
:::
...
[Ouvrir l'application dans un nouvel onglet](https://sitelf.fr/divers/ProgDyn/index.html){target="_blank"}

<iframe 
  src="https://sitelf.fr/divers/ProgDyn/index.html" 
  width="100%" 
  height="750px"
